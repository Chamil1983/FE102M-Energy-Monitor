...
ATM90E26_SPI::ATM90E26_SPI(int pin) {
  ...
  _ugain = 0xF765;  // Use XLS to calculate these values. Examples: 0xD464;  0xA028;(0xA03D)
  _igain = 0x6F4F;  // Use XLS to calculate these values. Examples: 0x6E49;  0x7DFB; (0x706C)
  
  _crc1 = 0xAE70;   // Run this application. Take auto calculated values and update here
  _crc2 = 0xF7BD;   // Run this application. Take auto calculated values and update here
...
void ATM90E26_SPI::InitEnergyIC() {
  ...
  CommEnergyIC(0, SagTh, 0x0F72);   // Voltage sag threshhold 0x17DD
  ...
  // Configure
  CommEnergyIC(0, MMode, 0x9422);  // Metering Mode Configuration. All defaults. See pg 31 of datasheet.(Default 0x9422)
  ...
  CommEnergyIC(0, QoffsetN, N_LineReactivePowerOffset);  // N line reactive power offset
...
void ATM90E26_SPI::InitCallibrate() {
    ...
    CommEnergyIC(0, FuncEn, 0x0030);  // Voltage sag irq=1, report on warnout pin=1, energy dir change irq=0
    CommEnergyIC(0, SagTh, VoltageSagThreshold);   // Voltage sag threshhold 0x1F2F
    ...
    CommEnergyIC(0, Lgain, L_LineCalibrationGain);     // Line calibration gain 0x1D39
    CommEnergyIC(0, Lphi, L_LineCalibrationAngle);      // Line calibration angle
    CommEnergyIC(0, Ngain, N_LineCalibrationGain);      // N Line calibration angle
    CommEnergyIC(0, Nphi, N_LineCalibrationAngle);      // N Line calibration angle
    CommEnergyIC(0, PStartTh, ActiveStartupPowerThreshold);  // Active Startup Power Threshold
    CommEnergyIC(0, PNolTh, ActiveNoLoadPowerThreshold);    // Active No-Load Power Threshold
    CommEnergyIC(0, QStartTh, ReactiveStartupPowerThreshold);  // Reactive Startup Power Threshold
    CommEnergyIC(0, QNolTh, ReactiveNoLoadPowerThreshold);    // Reactive No-Load Power Threshold
    CommEnergyIC(0, CSOne, Checksum1);      // Write CSOne, as self calculated

    // Set measurement calibration values
    CommEnergyIC(0, AdjStart, 0x5678);  // Measurement calibration startup command, registers 31-3A
    CommEnergyIC(0, Ugain, VoltagermsGain);     // Voltage rms gain
    CommEnergyIC(0, IgainL, L_LineCurrentrmsGain);    // L line current gain
    CommEnergyIC(0, IgainN, N_LineCurrentrmsGain);    // N line current gain
    CommEnergyIC(0, Uoffset, VoltageOffset);   // Voltage offset
    CommEnergyIC(0, IoffsetL, L_LineCurrentOffset);  // L line current offset
    CommEnergyIC(0, IoffsetN, N_LineCurrentOffset);  // N line current offset
    CommEnergyIC(0, PoffsetL, L_LineActivePowerOffset);  // L line active power offset
    CommEnergyIC(0, QoffsetL, L_LineReactivePowerOffset);  // L line reactive power offset
    CommEnergyIC(0, PoffsetN, N_LineActivePowerOffset);  // L line active power offset
    CommEnergyIC(0, QoffsetN, N_LineReactivePowerOffset);  // N line reactive power offset
    CommEnergyIC(0, CSTwo, Checksum2);      // Write CSTwo, as self calculated
