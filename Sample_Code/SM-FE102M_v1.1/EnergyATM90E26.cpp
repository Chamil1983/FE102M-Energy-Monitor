...
unsigned short ATM90E26_SPI::GetRegisterValue(unsigned char address) {
    return CommEnergyIC(1, address, 0xFFFF);
}
...
double ATM90E26_SPI::GetNLineCurrent() {
    unsigned short current1 = CommEnergyIC(1, IrmsTwo, 0xFFFF);
    return (double)current1 / 1000;
}
...
double ATM90E26_SPI::GetNLineActivePower() {
    short int apower1 = (short int)CommEnergyIC(1, PmeanTwo, 0xFFFF);  //Complement, MSB is signed bit
    return (double)apower1;
}
...
double ATM90E26_SPI::GetNLinePowerFactor() {
    short int pf1 = (short int)CommEnergyIC(1, PowerFTwo, 0xFFFF);  //MSB is signed bit
    //if negative
    if (pf1 & 0x8000) {
        pf1 = (pf1 & 0x7FFF) * -1;
    }
    return (double)pf1 / 1000;
}
...
double ATM90E26_SPI::GetNLineReactivePower() {
    short int rpower = (short int)CommEnergyIC(1, QmeanTwo, 0xFFFF); // Complement, MSB is signed bit
    return (double)rpower;
}
...
double ATM90E26_SPI::GetNLineApparentPower() {
    short int Apower1 = (short int)CommEnergyIC(1, SmeanTwo, 0xFFFF); // Complement, MSB is signed bit
    return (double)Apower1;
}

double ATM90E26_SPI::GetPhaseAngle() {
    short int pangle = (short int)CommEnergyIC(1, Pangle, 0xFFFF); // Complement, MSB is signed bit
    return (double)pangle/100;
}

double ATM90E26_SPI::GetNLinePhaseAngle() {
    short int pangle1 = (short int)CommEnergyIC(1, PangleTwo, 0xFFFF); // Complement, MSB is signed bit
    return (double)pangle1 / 100;
}
...
double ATM90E26_SPI::GetReactivefwdEnergy() {
    ...
    return (double)rpower / 10 / 1000;  //returns kVarh if PL constant set to 1000imp/kWh
}

double ATM90E26_SPI::GetReactivervsEnergy() {
    //Register is cleared after reading
    unsigned short rpower = CommEnergyIC(1, Rnenerg, 0xFFFF);  //0xFF, 0xFF, 0x42);
    return (double)rpower / 10 / 1000;  //returns kVarh if PL constant set to 1000imp/kWh
}
...
double ATM90E26_SPI::GetAbsReactiveEnergy() {
    ...
    return (double)abspower / 10 / 1000;  //returns kVarh if PL constant set to 1000imp/kWh
}
